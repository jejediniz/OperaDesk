...
async function request(
  path,
  { method = "GET", body, params, signal, headers = {}, timeout = DEFAULT_TIMEOUT_MS } = {}
) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const message =
        data?.error?.message || data?.message || `Erro ${response.status ?? "desconhecido"}`;
    ...
    const mutates = ["POST", "PUT", "PATCH", "DELETE"].includes(methodUpper);
    if (mutates && response.status !== 204 && (data === null || data === undefined)) {
      throw new HttpError("Resposta vazia ou JSON inválido do servidor. Tente de novo.", {
