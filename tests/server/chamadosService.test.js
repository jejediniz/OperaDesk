...
describe("chamadosService", () => {
  ...
  it("update bloqueia atribuição de técnico quando não permitido", async () => {
    await expect(
      chamadosService.update(1, { tecnicoId: 9 }, 7, {
        atualizarQualquer: false,
        podeAtribuir: false
      })
    ).rejects.toMatchObject({ statusCode: 403 });
  });
  ...
  it("update passa usuarioId quando não é atualização ampla", async () => {
    ...
    const result = await chamadosService.update(1, { status: "em_andamento" }, 7, {
      atualizarQualquer: false,
      podeAtribuir: false
    });

    expect(spy).toHaveBeenCalledWith(1, { status: "em_andamento" }, { usuarioId: 7 });
    expect(result?.id).toBe(1);
  });

  it("update passa usuarioId=null em atualização ampla (TI/admin)", async () => {
    const spy = vi.spyOn(chamadosRepository, "atualizarComDetalhes").mockResolvedValue({ id: 1 });

    await chamadosService.update(1, { status: "concluido" }, 7, {
      atualizarQualquer: true,
      podeAtribuir: true
    });

    expect(spy).toHaveBeenCalledWith(1, { status: "concluido" }, { usuarioId: null });
  });
});
